...
pub async fn start_discovery(
    ...
    tokio::spawn(async move {
        ...
        loop {
            interval.tick().await;
            if let Err(e) = send_discovery_message(socket_clone.clone(), &username_clone, local_addr).await {
    ...
    for addr_str in peer_addrs {
        ...
        if let Ok(addr) = SocketAddr::from_str(addr_str) {
            ...
            if is_new {
                ...
                let discovery_msg = Message::new_discovery(username.to_string(), local_addr);
                broadcaster::send_message(socket_clone.clone(), &discovery_msg, &addr.to_string()).await?;
            }
        }
    }
